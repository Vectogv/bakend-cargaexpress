...
export default class DisputeController {
  ...
  async storeRoot({ auth, request, response, serialize }: HttpContext) {
    ...
    if (!['esperando_confirmacion', 'completado', 'finalizado'].includes(viaje.estado)) {
      return response.status(422).send(serialize.withoutWrapping({ error: `Solo puedes disputar viajes en estado 'esperando_confirmacion', 'completado' o 'finalizado' (actual: ${viaje.estado})` }))
    }
  ...
  async store({ auth, params, request, response, serialize }: HttpContext) {
    ...
    if (!['esperando_confirmacion', 'completado', 'finalizado'].includes(viaje.estado)) {
      return response
        .status(422)
        .send(serialize.withoutWrapping({ error: `Solo puedes disputar viajes en estado 'esperando_confirmacion', 'completado' o 'finalizado' (actual: ${viaje.estado})` }))
    }
