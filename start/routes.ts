...
router
  .group(() => {
    ...
    router.get(':id/route', [controllers.Trip, 'route'])
...
router
  .group(() => {
    ...
    // Tickets de soporte: todas las zonas (?zona= y ?estado= para filtrar).
    router.get('tickets', [controllers.Ticket, 'staffIndex'])
    router.get('tickets/count', [controllers.Ticket, 'staffCount'])
    router.get('tickets/moderators', [controllers.Ticket, 'moderators'])
    router.get('tickets/:id', [controllers.Ticket, 'staffShow'])
    router.post('tickets/:id/take', [controllers.Ticket, 'take'])
    router.post('tickets/:id/messages', [controllers.Ticket, 'staffMessage'])
    router.put('tickets/:id/status', [controllers.Ticket, 'updateStatus'])
    router.put('tickets/:id/assign', [controllers.Ticket, 'assign'])
  })
  .prefix('/api/admin')
  .as('admin')
...
router
  .group(() => {
    ...
    router
      .post('tickets', [controllers.Ticket, 'store'])
      .use(middleware.rateLimit({ max: 10, windowMs: 60_000 }))
    router.post('tickets/upload', [controllers.Ticket, 'upload'])
    router.get('tickets/:id', [controllers.Ticket, 'show'])
    router.post('tickets/:id/messages', [controllers.Ticket, 'storeMessage'])
    router.post('tickets/:id/close', [controllers.Ticket, 'close'])
  })
  .prefix('/api/support')
  .as('support')
  .use(middleware.auth())
...
router
  .group(() => {
    ...
    // Tickets de soporte de la zona del moderador (?estado= para filtrar).
    router.get('tickets', [controllers.Ticket, 'staffIndex'])
    router.get('tickets/count', [controllers.Ticket, 'staffCount'])
    router.get('tickets/moderators', [controllers.Ticket, 'moderators'])
    router.get('tickets/:id', [controllers.Ticket, 'staffShow'])
